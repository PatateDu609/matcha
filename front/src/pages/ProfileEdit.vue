...
function onSubmit() {
  ...
  api.post<EditUserPayload>('/edit-user', data)
  .then(response => {
    ...
    userStore.fetchUser(userID).then( () => router.push('/profile'))
  })
        ...
        <q-card-section>
          <q-form class="column q-gutter-md" @submit="onSubmit">
            ...
            <div class="row no-wrap items-center">
              ...
              <q-item>
                  <q-input outlined v-model="bioText" label="Bio" />
              </q-item>
            </div>
            <div class="row no-wrap items-center">
              <q-item>
                  <q-select outlined v-model="genreSelect" :options="genreOptions" label="Genre *" />
              </q-item>
              <q-item>
